...
font = ("Times", 16)
doesMainMenuExist = False
connection = sqlite3.connect('./database1.db')
cursor = connection.cursor()
frame= Frame(root)   
def loginScreen(pframe):
    global doesMainMenuExist
    doesMainMenuExist = False
    pframe.destroy()
...
def mainMenu(userData,pFrame="Null"):
    global movingCanvas, doesMainMenuExist
    doesMainMenuExist = True
    if pFrame != "Null":
        pFrame.destroy()
    ...
    availableStudentsFrame=Frame(centeredFrame,relief=GROOVE,width=400,height=800,bd=3)                 
    availableStudentsFrame.grid(column=1,row=1,rowspan=4,padx=10,pady=10)
    availableStudentsFrame.grid_propagate(False)
            ...
            if len(classData) != 0:
                ...
                if (classData[i][4] == "paid"):
                    Button(frameForButtons,text="P",font=font,anchor=W,command = lambda i=i:
                           setStudentPaid(classData,i,"notPaid",frame,frameForButtons),bg = "light green").grid(row=i,column=5,padx=16,pady=2)
                else:
                    Button(frameForButtons,text="P",font=font,anchor=W,command = lambda i=i:
                           setStudentPaid(classData,i,"paid",frame,frameForButtons)).grid(row=i,column=5,padx=16,pady=2)                    
    else:
...
def moveCanvas(canvasForButtons):
    canvasForButtons.configure(scrollregion=canvasForButtons.bbox("all"),width=500,height=450)

def onMouseWheel(event):
    global movingCanvas, doesMainMenuExist
    if doesMainMenuExist == True:
        movingCanvas.yview_scroll(int(-1*(event.delta/120)), "units")
